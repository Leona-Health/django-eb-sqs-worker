...
def send_task(task_name, task_kwargs, run_locally=None, queue_name=None, delay_seconds=None):
    """
    ...
    if run_locally:
        ...
        if delay_seconds:
            if delay_seconds < 0:
                ...
                delay_seconds = 900
            response = queue.send_message(MessageBody=json.dumps(task_data), DelaySeconds=delay_seconds)
        else:
            response = queue.send_message(MessageBody=json.dumps(task_data))
        logger.info(f"Sent message {task_data} to SQS queue {queue_name} delaying {delay_seconds} seconds. Got response: {response}")
